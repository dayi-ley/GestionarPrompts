...
class CategoryGridFrame(QWidget):
    ...
    def save_as_new_character(self, variation_data, negative_prompt=""):
    ...
    def apply_variation(self, variation_data):
        ...
        if loaded_count > 0:
            ...
            QMessageBox.information(
                self, 
                "Variación cargada", 
                f"Se han aplicado {loaded_count} categorías de '{variation_name}'."
            )
...
class ImportDataDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.text_area.setPlaceholderText(
            "Ejemplo:\n"
            "Personaje: chica anime\n"
            "Cabello color: rubio\n"
            "Vestuario general: uniforme escolar\n"
            "..."
        )
    ...
    def process_import(self):
        ...
        if not text:
            QMessageBox.warning(self, "Error", "No hay datos para importar")
            return
        ...
        if not mapped_data:
            QMessageBox.warning(self, "Error", "No se encontraron datos válidos para importar")
            return
        ...
        if categories_empty:
            reply = QMessageBox.question(
                ...
                f"Se encontraron {len(categories_with_values)} categorías con valores y {len(categories_empty)} categorías vacías.\n\n"
    ...
    def apply_preset(self, preset_data):
        ...
        # Obtener las categorías del preset (es un diccionario, no una lista)
        ...
        # Limpiar solo las categorías que están en el preset y aplicar los valores
        for card in self.cards:
            if hasattr(card, 'category_name') and hasattr(card, 'input_field'):
                if card.category_name in preset_categories:
                    # Aplicar el valor del preset a la categoría
                    preset_value = preset_categories[card.category_name]
        ...
        QMessageBox.information(
            ...
            f"Categorías actualizadas: {len(preset_categories)}"
        )
